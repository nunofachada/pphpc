...
int main(int argc, char ** argv)
{
	...
	CLZONE zone = parseArgs(argc, argv);
	...
	// Inform user of fixed kernel sizes
	...
	// 4. Create and initialize host buffers
	// Statistics
	STATS statistics;
...
// Parse arguments and obtain clzone
CLZONE parseArgs(int argc, char ** argv) {
	if (argc != 2) {
		printf("Usage: %s cpu|gpu\n", argv[0]);
		exit(1);
	} else if (strcmp(argv[1], "cpu") == 0) {
		return getClZone("Advanced Micro Devices, Inc.", "CPUPredPrey_Kernels.cl", CL_DEVICE_TYPE_CPU);
	} else if (strcmp(argv[1], "gpu") == 0) {
		return getClZone("NVIDIA Corporation", "GPUPredPrey_Kernels.cl", CL_DEVICE_TYPE_GPU);
	} else {
		printf("Usage: %s cpu|gpu\n", argv[0]);
		exit(1);
	}
