...
__kernel void bsort_init(__global uagr4 *g_data, __local uagr4 *l_data) {

	int dir;
	uint id, global_start, size, stride;
	uagr4 input1, input2, temp;
	int4 comp;
...
__kernel void bsort_stage_0(__global uagr4 *g_data, __local uagr4 *l_data, 
									 uint high_stage) {
...
__kernel void bsort_stage_n(__global uagr4 *g_data, __local uagr4 *l_data, 
...
__kernel void bsort_merge(__global uagr4 *g_data, __local uagr4 *l_data, uint stage) {
...
__kernel void bsort_merge_last(__global uagr4 *g_data, __local uagr4 *l_data) {
	...
	for(stride = get_local_size(0)/2; stride > 1; stride >>= 1) {
		barrier(CLK_LOCAL_MEM_FENCE);
		id = get_local_id(0) + (get_local_id(0)/stride)*stride;
		VECTOR_SWAP(l_data[id], l_data[id + stride], DIRECTION)
	}
	barrier(CLK_LOCAL_MEM_FENCE);
